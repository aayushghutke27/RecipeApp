...
import com.example.recipeapp.databinding.ItemCardRecipeWeekBinding

class RecipeOfWeekAdapter(
    private var recipes : List<Recipe>,
    private val onRecipeClick: (Recipe) -> Unit,
    private val onFavoriteClick: ((Recipe) -> Unit) ?= null,
    private val isFavorite: ((String) -> Boolean) ?= null
) : RecyclerView.Adapter<RecipeOfWeekAdapter.RecipeWeekViewHolder>() {
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): RecipeWeekViewHolder {
        val binding =
            ItemCardRecipeWeekBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return RecipeWeekViewHolder(binding)
    }

    override fun onBindViewHolder(
        holder: RecipeWeekViewHolder,
        position: Int
    ) {
    ...
    inner class RecipeWeekViewHolder(private val binding: ItemCardRecipeWeekBinding) :
